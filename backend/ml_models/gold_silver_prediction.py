...
def _build_explanations(feature_cols, X, lr_gld, lr_slv):
    empty = {
        "shap": {
            "features": feature_cols,
            "gld_importance": [0.0 for _ in feature_cols],
            "slv_importance": [0.0 for _ in feature_cols],
        },
        "lime": {
    ...
    try:
        ...
        lime_explainer_gld = lime.lime_tabular.LimeTabularExplainer(
            X, feature_names=feature_cols, class_names=['GLD_Price'], mode='regression'
        )
        exp_gld = lime_explainer_gld.explain_instance(X[-1], lr_gld.predict, num_features=len(feature_cols))

        lime_explainer_slv = lime.lime_tabular.LimeTabularExplainer(
            X, feature_names=feature_cols, class_names=['SLV_Price'], mode='regression'
        )
        exp_slv = lime_explainer_slv.explain_instance(X[-1], lr_slv.predict, num_features=len(feature_cols))
        ...
        logger.warning("Gold/silver multivariate explainability failed; returning empty explanations: %s", exc)
        return empty

def analyze_gold_silver_multivariate(interval='1y'):
    """
    Fetches, predicts, and returns Gold & Silver data for the given interval.
    interval: '1d', '1mo', '3mo', or '1y'
    Returns a dictionary matching the previous JSON structure.
    """
    print(f"Fetching data from Yahoo Finance for GLD and SLV... (Interval: {interval})")
    ...
    # For 'YE', year is fine. For others, we'll use an integer index representing time steps.
    df['TimeIndex'] = range(len(df))
    
    # 3.b. Feature Engineering
    df['GLD_Lag_1'] = df['GLD'].shift(1)
    df['SLV_Lag_1'] = df['SLV'].shift(1)
    
    df['GLD_Lag_2'] = df['GLD'].shift(2)
    df['SLV_Lag_2'] = df['SLV'].shift(2)
    
    df['Gold_Silver_Ratio_Lag_1'] = df['GLD_Lag_1'] / (df['SLV_Lag_1'] + 1e-9)
    
    df = df.dropna().reset_index(drop=True)
    
    feature_cols = ['TimeIndex', 'GLD_Lag_1', 'SLV_Lag_1', 'GLD_Lag_2', 'SLV_Lag_2', 'Gold_Silver_Ratio_Lag_1']
    ...
    future_time_indices = [last_time + i for i in range(1, prediction_steps + 1)]
    
    # Properly generate future dates
    ...
    current_gld = df.iloc[-1]['GLD']
    current_slv = df.iloc[-1]['SLV']
    current_gld_lag1 = df.iloc[-1]['GLD_Lag_1']
    current_slv_lag1 = df.iloc[-1]['SLV_Lag_1']
    
    for i, target_time in enumerate(future_time_indices):
        ratio_lag = current_gld / (current_slv + 1e-9)
        
        walk_forward_features = np.array([[
            target_time,
            current_gld,
            current_slv,
            current_gld_lag1,
        ...
        next_gld = lr_gld.predict(walk_forward_features)[0]
        next_slv = lr_slv.predict(walk_forward_features)[0]
        
        pred_gld_list.append(next_gld)
        pred_slv_list.append(next_slv)
        
        current_gld_lag1 = current_gld
        current_slv_lag1 = current_slv
        current_gld = next_gld
        current_slv = next_slv

    predictions_df = pd.DataFrame({
        'Date': future_dates,
        'TimeIndex': future_time_indices,
        'Predicted_GLD': pred_gld_list,
        'Predicted_SLV': pred_slv_list
    })
    
    explanations = _build_explanations(feature_cols, X, lr_gld, lr_slv)

    # Format JSON payload
    historical_data = []
    # To avoid returning massive datasets for "1d", let's limit historical returned to the frontend.
    display_history_limit = 100 if interval == '1d' else 200
    df_recent = df.tail(display_history_limit)
    
    for index, row in df_recent.iterrows():
        # Represent Date nicely based on interval
        date_str = str(row['Date'].date())
        # Provide compatible 'Year' prop for existing components as fallback label
